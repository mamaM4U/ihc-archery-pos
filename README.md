...
- Upgrade ke Laravel 13 + PHP 8.3+ (tested on PHP 8.5)
- Laravel Boost v2 untuk AI-assisted development (MCP server)
- Spatie Laravel Permission v7
- PHPUnit 12
- Laravel Tinker v3
- Laravolt Indonesia v0.41 (support Laravel 13)
...
- [Laravel Boost](https://laravel.com/docs/boost) (AI-assisted development via MCP)
...
Project ini menyertakan [Laravel Boost](https://laravel.com/docs/boost) sebagai MCP server untuk AI coding assistant (Kiro, Claude Code, Cursor, dll). Boost memberikan konteks project ke AI: database schema, routes, logs, dan dokumentasi Laravel yang version-specific.

Setup setelah `composer install`:

```bash
php artisan boost:install
```

Perintah di atas akan generate config MCP dan AI guidelines sesuai editor yang kamu pilih. Pastikan `APP_ENV=local` di `.env` karena Boost hanya aktif di environment local.

Untuk Kiro, tambahkan file `.kiro/settings/mcp.json`:
...
Ganti `/path/to/php` dengan output dari `which php` dan `cwd` dengan path project kamu.
