...
export default function Show({
    ...
    const persistItem = (item) => {
        ...
        if (item.physical_stock !== null && difference !== 0 && !item.adjustment_reason) {
            toast.error("Alasan wajib diisi jika ada selisih stok!");
            return;
        }
        ...
        router.patch(
            ...
            },
            {
                ...
                onError: (errors) => {
                    ...
                    toast.error(firstError || "Gagal memperbarui item opname");
                },
    ...
    const finalize = () => {
        router.post(
            ...
            {},
            {
                ...
                onError: (errors) => {
                    const firstError = Object.values(errors)[0];
                    toast.error(firstError || "Gagal finalize. Periksa item yang belum valid.");
                },
            }
        );
    };
                                            ...
                                            >
                                                ...
                                                <Table.Td>
                                                    <input
                                                        ...
                                                        className={`h-10 w-full min-w-48 rounded-lg border px-3 text-sm text-slate-800 outline-none transition focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-800 dark:text-slate-200 ${
                                                            isDifferent && !item.adjustment_reason
                                                                ? "border-red-500 bg-red-50 focus:border-red-500 focus:ring-red-500/20 dark:border-red-500/50 dark:bg-red-950/20"
                                                                : "border-slate-200 bg-slate-50 focus:border-primary-500 focus:ring-primary-500/20 dark:border-slate-700"
                                                        }`}
                                                    />
                                                </Table.Td>
                                                <Table.Td className="text-center">
                                                    {isDraft ? (
                                                        <button
                                                            ...
                                                            disabled={savingItemId === item.id || (isDifferent && !item.adjustment_reason)}
