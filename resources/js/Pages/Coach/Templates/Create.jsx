...
export default function Create({ coaches }) {
    const { auth } = usePage().props;
    const isAdmin = auth.user.role === "admin";

    const [selectedCoach, setSelectedCoach] = useState(
        isAdmin ? null : { id: auth.user.id, name: auth.user.name }
    );

    // Initial state for form
    const { data, setData, post, errors, processing } = useForm({
        coach_id: isAdmin ? "" : auth.user.id,
        template_name: "",
        booking_open_days: 7,
        is_active: true,
        notes: "",
    ...
    const submit = (e) => {
        ...
        // Extra client-side check for admin to select a coach
        ...
        post(route("templates.store"), {
            onSuccess: () => {
                toast.success("Template mingguan berhasil disimpan");
            },
            onError: () => {
                toast.error("Gagal menyimpan template. Silakan periksa input Anda.");
            },
        });
    };
    ...
    return (
        <>
            <Head title="Tambah Template Jadwal" />
            ...
            <div className="mb-6">
                ...
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    ...
                    Tambah Template Jadwal Baru
                </h1>
                <p className="text-sm text-slate-500 mt-1">
                    Buat template mingguan baru. Anda dapat menambahkan beberapa sesi latihan sekaligus dalam template ini.
                </p>
            </div>
